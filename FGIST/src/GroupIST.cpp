...
//objective function value
  ...
  do {
    ...
    for(i=0 ; i < n/2; i++){
      ...
      if(l1(i) != 0){
        beta1(i) =  kesi1(i) * Thetaholding(l1(i),lambda,eta)/l1(i);
        beta1(i+n/2) = kesi1(i+n/2)*Thetaholding(l1(i),lambda,eta)/l1(i);
      }else{
...
//to calculate Penalty term
  ...
  for(i=0 ; i < n/2; i++){
    Penaltyterm=Penaltyterm+Penalty(sqrt(beta1(i)*beta1(i)+beta1(i+n/2)*beta1(i+n/2)),lambda,eta);
  }
